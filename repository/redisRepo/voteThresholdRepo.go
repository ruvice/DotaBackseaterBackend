...
func (r *RedisRepo) GetVoteThreshold(ctx context.Context, channelID string) (string, *DBSError.VoteError) {
	key := "voteThreshold:" + channelID
	log.Println("In Redis GetVoteThreshold", key)
	voteThreshold, err := r.Client.Get(ctx, key).Result()
	if err == redis.Nil {
		fmt.Printf("Could not find key: %v\n", err)
		return "", DBSError.NewError(DBSError.CodeMissingCacheVoteThreshold, "Could not find vote threshold cache")
	} else if err != nil {
		fmt.Printf("Error getting key: %v\n", err)
		return "", DBSError.NewError(DBSError.CodeUnknown, "Error getting key")
	}
